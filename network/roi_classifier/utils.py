...
def make_detections_valid(cfg, detections):
    ...
    detections_valid[detections_valid[:, 4] <= 40, 4] = 40
    detections_valid[detections_valid[:, 3] >= cfg["heatmap"]["output_dimension"] - 1, 3] = cfg["heatmap"]["output_dimension"] - 1
    detections_valid[detections_valid[:, 4] >= cfg["heatmap"]["output_dimension"] - 1, 4] = cfg["heatmap"]["output_dimension"] - 1
    for i in range(detections.shape[0]):
        ...
        if (x > cfg["heatmap"]["output_dimension"] - 1):
            detections_valid[i, 1] = cfg["heatmap"]["output_dimension"] - 1 - detections_valid[i, 3]
        if (y > cfg["heatmap"]["output_dimension"] - 1):
            detections_valid[i, 2] = cfg["heatmap"]["output_dimension"] - 1 - detections_valid[i, 4]
