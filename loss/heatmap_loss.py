...
def calculate_heatmap_loss(predicted_heatmap, groundtruth_heatmap):
    ...
    heatmap_loss = torch.nn.functional.mse_loss(
        input=predicted_heatmap.float(),
        target=groundtruth_heatmap.float(),
        reduction="mean",
    )
    return heatmap_loss


def calculate_heatmap_scatter_loss(
    predicted_heatmap, groundtruth_heatmap, flattened_index, num_objects, device
):
    ...
    object_boolean_mask = (
        object_boolean_mask.unsqueeze(2).expand_as(predicted_heatmap).float()
    )
    predicted_heatmap, groundtruth_heatmap = (
        predicted_heatmap * object_boolean_mask,
        groundtruth_heatmap * object_boolean_mask,
    )
    bbox_loss = F.smooth_l1_loss(
        predicted_heatmap, groundtruth_heatmap, reduction="sum"
    )
