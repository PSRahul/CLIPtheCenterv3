...
def perform_nms(cfg, prediction):
    ...
    for image_id in tqdm(image_id_list):
        ...
        nms_ind = nms(
            boxes, scores, iou_threshold=cfg["post_processing"]["nms_iou_threshold"]
        ).numpy()

        # columns = [["image_id", "bbox_x", "bbox_y", "w", "h", "score", "class_label"]]
        # Filter based on the scores
        scores_filtered = scores[nms_ind]
        filter_by_nms_and_score_ind = (
            scores_filtered > cfg["post_processing"]["score_threshold"]
        )
