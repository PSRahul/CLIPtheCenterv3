...
def generate_gaussian_peak(cfg, height, width):
    ...
    m, n = [(ss - 1.0) / 2.0 for ss in (gaussian_diameter, gaussian_diameter)]
...
def generate_gaussian_heatmap(
    cfg, h, w, bbox_center_int, threshold, set_constant_value=0, normalise=False
):
    # This will generate a gaussian map in the output dimension size
    object_heatmap = np.zeros(
        (cfg["heatmap"]["output_dimension"], cfg["heatmap"]["output_dimension"])
    )
    ...
    left, right = min(bbox_center_int[0], gaussian_radius), min(
        output_width - bbox_center_int[0], gaussian_radius + 1
    )
    top, bottom = min(bbox_center_int[1], gaussian_radius), min(
        output_height - bbox_center_int[1], gaussian_radius + 1
    )

    masked_object_heatmap = object_heatmap[
        bbox_center_int[1] - top : bbox_center_int[1] + bottom,
        bbox_center_int[0] - left : bbox_center_int[0] + right,
    ]
    masked_gaussian_peak = gaussian_peak[
        gaussian_radius - top : gaussian_radius + bottom,
...
def create_heatmap_object(cfg, heatmap_bounding_box):
    ...
    bbox = np.array(
        [
            heatmap_bounding_box[0],
            heatmap_bounding_box[1],
    ...
    bbox_heatmap_h = generate_gaussian_heatmap(
        cfg,
        bbox_h,
        bbox_w,
        bbox_center,
    ...
    bbox_heatmap = np.vstack(
        (np.expand_dims(bbox_heatmap_w, axis=0), np.expand_dims(bbox_heatmap_h, axis=0))
    )
