...
class DataModule():
    ...
    def load_train_dataloader(self):
        return DataLoader(self.load_train_dataset(),drop_last=True, batch_size=self.cfg["data"]["train_batch_size"], shuffle=True,num_workers=4)

    def load_val_dataloader(self):
        return DataLoader(self.load_val_dataset(),drop_last=True, batch_size=self.cfg["data"]["val_batch_size"], shuffle=True,num_workers=4)

    def load_test_dataloader(self):
        return DataLoader(self.load_test_dataset(),batch_size=self.cfg["data"]["test_batch_size"],shuffle=False,num_workers=4)
